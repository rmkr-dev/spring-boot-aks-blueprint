...
- **HTTP security headers.** `SecurityHeadersFilter` sets `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Permissions-Policy`, `Cache-Control: no-store`, and disables legacy `X-XSS-Protection` on responses. See [http-headers.md](http-headers.md).
