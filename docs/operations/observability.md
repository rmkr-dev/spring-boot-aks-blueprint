...
`BlueprintInfoContributor` adds a `blueprint` object to `/actuator/info` (stack label, Java version, purpose). Combined with `info.app.*` from `application.yml` and Maven `build-info` (`build.artifact` / `build.version`) generated by `spring-boot-maven-plugin`.
