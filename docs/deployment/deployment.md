...
# or
mvn -B package && java -jar target/spring-boot-aks-blueprint-1.2.1.jar
```
...
The sample Deployment also sets `minReadySeconds: 10` and `terminationGracePeriodSeconds: 45` and a 5s `preStop` sleep to align with `server.shutdown=graceful` and a 30s Spring lifecycle timeout (see [graceful-shutdown.md](../operations/graceful-shutdown.md)).
