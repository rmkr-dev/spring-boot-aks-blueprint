...
| `src/test/java/...` | Unit + WebMvc + Actuator + error + filter + graceful-shutdown / preStop manifest tests | Present |
...
Configuration prefers environment variables over hardcoding (`SERVER_PORT`, `SPRING_APPLICATION_NAME`). Virtual threads are enabled via `spring.threads.virtual.enabled=true` (Java 21). Tomcat `connection-timeout`/`keep-alive-timeout` are set explicitly for honest defaults. `server.forward-headers-strategy=framework` supports Ingress/`X-Forwarded-*` when a proxy is present. Health probes are enabled for Kubernetes samples; health details are not shown anonymously. There is no authentication layer and no datastore in this blueprint. Springdoc/OpenAPI is deferred—see [api-errors.md](../development/api-errors.md).
